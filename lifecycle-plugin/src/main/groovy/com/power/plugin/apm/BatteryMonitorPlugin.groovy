...
class BatteryMonitorPlugin extends Transform implements Plugin<Project> {
    ...
    void transform(@NonNull TransformInvocation transformInvocation) {
        ...
        println "PowerMonitorPlugin start"
        ...
        println "PowerMonitorPlugin end, cost ： $cost s"
    }
    ...
    static void handleDirectoryInput(DirectoryInput directoryInput, TransformOutputProvider outputProvider) {
        ...
        if (directoryInput.file.isDirectory()) {
            ...
            directoryInput.file.eachFileRecurse { File file ->
                def name = file.name
                if (name.contains("RealCall")) {
                    println "okhttp name= $name"
                }
                if (checkClassFile(name)
//                        && (entryName.equals("RealCall.class") || PreReadClassImport.containImport(file,
//                        filerImportClassName))
                ) {
    ...
    static void handleJarInputs(JarInput jarInput, TransformOutputProvider outputProvider) {
        if (jarInput.file.getAbsolutePath().endsWith(".jar")) {
            ...
            while (enumeration.hasMoreElements()) {
                ...
                if (entryName.contains("RealCall")) {
                    println "okhttp name= $entryName"
                }
                //插桩class
                if (checkClassFile(entryName)
//                        && (entryName.equals("RealCall.class") ||
//                        PreReadClassImport.containImport(inputStream1,
//                                filerImportClassName))
                ) {
                    //class文件处理
